...
def get_arg_parser():
    ...
    parser.add_option("-n","--name", dest="name_tag", 
                      help="'Name' tag to be applied to snapshot.  Does not override 'Name' from volume.  Must be supplied if 'Name' tag is not set on volume being snapshotted")
    return parser
...
if __name__ == "__main__":
    ...
    if volume_id is None or "vol" not in volume_id.strip():
        raise Exception("Invalid argument for volume_id.  Please provide an argument of the volume_id to be snapshotted.")

    ec2 = boto.connect_ec2()

    #Determine if we need to set the name or not based on the target volume
    volumes = filter(lambda boto_volume: volume_id == boto_volume.id, ec2.get_all_volumes())
    volume = volumes[0] if len(volumes) > 0 else None
    if volume is None: raise Exception(volume_id + " not found in volumes for account")

    #Make sure a 'Name' tag gets created for the snapshot and assign it to local variable
    name_tag = None
    if "Name"  in volume.tags:
        ...
        if name_tag is None:
            raise Exception("passed in volume must have a 'Name' tag or a name must be passed in with the --name option.")
    ...
    #Finally, trim all snapshots.  Modify arguments to this call if default settings need to be overriden.
    ec2.trim_snapshots()
